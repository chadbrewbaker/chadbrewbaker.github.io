...
static u32 insert(Rel *r, const u32 *t) {
  if (r->arity == 0) {
...
EXPORT(dl_fact) i32 dl_fact(u32 pred) {
  ...
  return (i32)insert(&rels[pred], scratch_buf);
}
...
// Build:
//   clang --target=wasm32 -O2 -nostdlib -Wl,--no-entry -Wl,-z,stack-size=131072 \
//         -o horn.wasm horn.c
// then base64-encode horn.wasm into the WASM_B64 constant in horn-solver.html.
